...
Full detail on each in `TEST_PLAN_1.5.8.md` and the 1.5.5-1.5.13 entries in `README.md`. The current test plan is always named for the version under test (now `TEST_PLAN_1.5.14.md`) and no longer duplicates confirmed items - it only lists tests still outstanding (currently EFFCLR-01/02/03/04).
